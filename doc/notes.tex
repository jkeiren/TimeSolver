...
This bad region can be identified by $\mathit{bad} \equiv (\suc((\loc, \region)) \cap \pre_{<}(\placeholder[2])) \setminus \placeholder[1]$. Note that bad now contains all states in
$\suc((\loc, \region)) \cap \pre_{<}(\placeholder[2])$ that are
not in $\placeholder[1]$, and therefore all states in $\suc((\loc, \region)) \cap \pre_{<}(\placeholder[2])$ not in $\suc((\loc, \region)) \cap \placeholder[1]$ \todo{Prove}.
...
$\suc((\loc, \region)) \cap \pre_<(\placeholder) \subseteq \suc((\loc, \region)) \cap \placeholder[1]$. Since $\placeholder$ excludes all elements from $\suc((\loc, \region)) \cap \pre(\mathit{bad})$, this follows from the definition of $\mathit{bad}$.
...
The bad part of $\placeholder[2]^1$ is indicated in red; the good part is indicated in
...
\begin{algorithm}[H]
  ...
  \ELSE
    ...
    \FOR{\textbf{each} $\placeholder[2]^i$ in $\placeholder[2]$}
      ...
      \STATE $\placeholder^i \gets \placeholder[2]^i \cap \overline{\suc((\loc, \region) \cap \pre(\var{bad})}$%$ \cap \placeholder[1]$
