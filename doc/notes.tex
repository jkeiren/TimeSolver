...
\todo{Extend with an example that shows we also need to purge all cache entries for sequents above this sequent in the proof tree.}
\todo{What is the cache-invariant that is preserved here?}
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
  \STATE \COMMENT{Assume $\phi = X$, and $X \stackrel{\mu/\nu}{=} \phi_X \in \mathcal{E}$}
  \STATE \COMMENT{There are 4 caches: $C_\top$, $C_\bot$, $C_\mu$, $C_\nu$, that contain known-true and knwon-false sequents, and previously detected least- and greatest fixed point sequents, respectively.}
  \IF{$(X, \loc, \region) \in C_\top$}
    \RETURN $\mathit{true}$
  \ELSIF{$(X, \loc, \region) \in C_\bot$}
    \RETURN $\mathit{false}$
  \ENDIF
  ...
  \STATE \COMMENT{Update the caches; sometimes we need to update cached values since other values depend on the current value.}
...
\subsection{Some background in the literature}
The implementation is based on the works by Fontana \cite{FC:14,FC:14report,Fontana:14} and Zhang~\cite{ZC:05forte,ZC:05rtss,Zhang:05}.
