...
The first of these is a side condition that we will use, among others, for relativized exists. Given a region $(\loc, \region)$, placeholders $\placeholder[1]$ and $\placeholder[2]$, we need to find the largest placeholder $\placeholder[\exists]$ such that $(\loc, \region) \cap \placeholder[\exists]$ and $\suc((\loc, \region) \cap \placeholder[\exists]) \cap \pre_{<}(\placeholder[2]) \subseteq \suc((\loc, \region) \cap \placeholder[\exists]) \cap \placeholder[1]$.
...
\end{tikzpicture}
\end{center}
In particular, the condition is not satisfied due to the triangle in which $\suc((\loc, \region))$ and $\pre_{<}(\placeholder[2])$ overlap, which is not included in $\placeholder[1]$. To eliminate this, we must restrict $\placeholder[\exists]$ such that $\succ((\loc, \region) \cap \placeholder[\exists])$ does not include this \emph{bad} triangle.

This bad region can be identified by $\mathit{bad} \equiv \suc((\loc, \region)) \cap \pre_{<}(\placeholder[2]) \cap \overline{\placeholder[1]}$, where $\overline{S}$ denotes the set-complement of $S$. The bad part of $(\loc, \region)$ can now be established as $(\loc, \region) \cap \pre(\mathit{bad}) \equiv (\loc, \region) \cap \pre(\suc((\loc, \region)) \cap \pre_{<}(\placeholder[2]) \cap \overline{\placeholder[1]})$.
...
If, in contrast, we would like to find the \emph{good} part of $(\loc, \region)$, and assign this to $\placeholder[\exists]$, we compute
$(\loc, \region) \cap \overline((\loc, \region) \cap \pre(\mathit{bad}))$, which simplifies to $(\loc, \region) \cap \overline{\pre(\mathit{bad})} \equiv (\loc, \region) \cap \overline{\pre(\suc((\loc, \region)) \cap \pre_{<}(\placeholder[2]) \cap \overline{\placeholder[1]})}$

The resulting $\placeholder[\exists]$ satisfies the conditions. This situation is shown in the following figure.
...
\draw[fill=red!30] (0,0) -- (0,1) -- (1,1) -- (0,0);
\node[color=red] at (0.25,0.5) {$\placeholder[\exists]$};
