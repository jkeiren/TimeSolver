...
can be computed from this as $\succ_{>}(\mathit{bad}$.
...
\subsubsection{An interesting, linear example (corner case)}
...
\begin{algorithm}[H]
  ...
  \ELSE
    ...
    \FOR{\textbf{each} $\placeholder[2]^i$ in $\placeholder[2]$}
      ...
      \STATE $\placeholder^i \gets \placeholder[2]^i \cap \overline{\suc(\var{bad})}$%$ \cap \placeholder[1]$
