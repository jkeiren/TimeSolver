...
bool prover::do_proof(int step, DBM * const lhs, const ExprNode * const rhs, SubstList * const sub)
{
  ...
  switch(rhs->getOpType()){
    ...
    default:
    {
      cpplog(cpplogging::debug) <<"Error, operator not defined." << endl;
      assert(false);
    }
  }
                         ...
                         const ExprNode * const rhs, SubstList * const sub)
{
  ...
  switch(rhs->getOpType()){
    case PREDICATE:{
      retPlaceDBM = do_proof_place_predicate(step, lhs, place, rhs, sub);
      break;
    }
    case AND:{
      retPlaceDBM = do_proof_place_and(step, lhs, place, rhs, sub);
      break;
    }
    case OR:{
      retPlaceDBM = do_proof_place_or(step, lhs, place, rhs, sub);
      break;
    }
    case OR_SIMPLE:{
      retPlaceDBM = do_proof_place_or_simple(step, lhs, place, rhs, sub);
      break;
    }
    case FORALL:{
      retPlaceDBM = do_proof_place_forall(step, lhs, place, rhs, sub);
      break;
    }
    case FORALL_REL: {
      retPlaceDBM = do_proof_place_forall_rel(step, lhs, place, rhs, sub);
      break;
    }
    case EXISTS:{
      retPlaceDBM = do_proof_place_exists(step, lhs, place, rhs, sub);
      break;
    }
    case EXISTS_REL: {
      retPlaceDBM = do_proof_place_exists_rel(step, lhs, place, rhs, sub);
      break;
    }
    case ALLACT: {
      retPlaceDBM = do_proof_place_allact(step, lhs, place, rhs, sub);
      break;
    }
    case EXISTACT: {
      retPlaceDBM = do_proof_place_existact(step, lhs, place, rhs, sub);
      break;
    }
    case IMPLY:{
      retPlaceDBM = do_proof_place_imply(step, lhs, place, rhs, sub);
      break;
    }
    case CONSTRAINT:{
      retPlaceDBM = do_proof_place_constraint(step, lhs, place, rhs, sub);
      break;
    }
    case BOOL:{
      retPlaceDBM = do_proof_place_bool(step, lhs, place, rhs, sub);
      break;
    }
    case ATOMIC:{
      retPlaceDBM = do_proof_place_atomic(step, lhs, place, rhs, sub);
      break;
    }
    case ATOMIC_NOT:{
      retPlaceDBM = do_proof_place_atomic_not(step, lhs, place, rhs, sub);
      break;
    }
    case ATOMIC_LT:{
      retPlaceDBM = do_proof_place_atomic_lt(step, lhs, place, rhs, sub);
      break;
    }
    case ATOMIC_GT:{
      retPlaceDBM = do_proof_place_atomic_gt(step, lhs, place, rhs, sub);
      break;
    }
    case ATOMIC_LE:{
      retPlaceDBM = do_proof_place_atomic_le(step, lhs, place, rhs, sub);
      break;
    }
    case ATOMIC_GE:{
      retPlaceDBM = do_proof_place_atomic_ge(step, lhs, place, rhs, sub);
      break;
    }
    case SUBLIST:{
      retPlaceDBM = do_proof_place_sublist(step, lhs, place, rhs, sub);
      break;
    }
    case RESET:{
      retPlaceDBM = do_proof_place_reset(step, lhs, place, rhs, sub);
      break;
    }
    case ASSIGN:{
      retPlaceDBM = do_proof_place_assign(step, lhs, place, rhs, sub);
      break;
    }
    case REPLACE:{
      retPlaceDBM = do_proof_place_replace(step, lhs, place, rhs, sub);
      break;
    }
    case ABLEWAITINF:{
      retPlaceDBM = do_proof_place_ablewaitinf(step, lhs, place, rhs, sub);
      break;
    }
    case UNABLEWAITINF:{
      retPlaceDBM = do_proof_place_unablewaitinf(step, lhs, place, rhs, sub);
      break;
    }
    default:
      cpplog(cpplogging::debug) <<"ERROR, OPERATOR NOT DEFINED" << endl;
