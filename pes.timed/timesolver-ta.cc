...
void printUsage(){
  ...
  std::cerr << "\t option: -d  print debug information which includes the proof tree" << endl;
  std::cerr << "\t option: -t print out the end caches of sequents" << endl;
  std::cerr << "\t option: -h  this help info" << endl;
  std::cerr << "\t option: -n (no caching) disables performance-optimizing known true and known false caches. Circularity stack caching still used." << endl;
}
...
int main(int argc, char** argv){
  ...
  if (!yyin) {
    cpplog(cpplogging::error) << "Cannot open input file " << opt.input_filename << std::endl;
    exit(-1);
  }
  ...
  if(parseError) {
    cpplog(cpplogging::error) << endl << "**Syntax Error: Error Parsing file.**" << endl << endl;
    cpplog(cpplogging::error) << "==--End of Program Execution-----------------------==" << endl;
    return 1;
  }
  ...
  cpplog(cpplogging::info) << "Program input file (timed automaton + MES): " << opt.input_filename << endl;
  cpplog(cpplogging::info) << "max constant in clock constraints: " << MAXC << endl << endl;
  ...
  if(start == nullptr){
    cpplog(cpplogging::error) << "Start predicate is not properly given." << endl;
    exit(-1);
  }
  ...
  SubstList sublist(input_pes.atomic().size(), input_pes.atomic());
  for(size_t i=0; i < input_pes.atomic().size(); i++){
  ...
  DBM dbm(input_pes.spaceDimension(), input_pes.clocks());
  ...
  time(&rawtime);
  /* print Starting time */ 
  cpplog(cpplogging::debug) << "##--Beginning of Proof==-------------------" << endl
  ...
  prover p(input_pes, currParityGfp, prevParityGfp,
           opt.useCaching, opt.nHash, MAXC, opt.nbits);
  ...
  cpplog(cpplogging::debug) << "##--End of Proof==------------------" << endl << endl;
  if (suc) {
    cpplog(cpplogging::info) << "--==Program Result:  **Valid**  ==-------------------" << endl;
    cpplog(cpplogging::info) << "Valid proof found. The timed automaton satisfies the MES." << endl << endl;
  }
  else {
    cpplog(cpplogging::info) << "--==Program Result:  **Invalid**  ==-----------------" << endl;
    cpplog(cpplogging::info) << "Invalid proof found. The timed automaton does not satisfy the MES." << endl << endl;
  }

  double totalTime = clockToSecs*(end_lp + begin_lp) + clockToSecs*(end - begin);
  
  cpplog(cpplogging::info) << "--==Program Time:  **" << clockToSecs*(end_lp + begin_lp) + clockToSecs*(end - begin) << " seconds**  ==----------" << endl;
  cpplog(cpplogging::info) << "Lexer and parser running time: " << clockToSecs*(end_lp - begin_lp) << " seconds" << endl;
  cpplog(cpplogging::info) << "Prover running time: " << clockToSecs*(end - begin) << " seconds" << endl;
  cpplog(cpplogging::info) << "Combined running time: " << totalTime  << " seconds" << endl;
  
  cpplog(cpplogging::info) << "Number of locations: " << p.getNumLocations() << endl;
  ...
  cpplog(cpplogging::info) << "==--End of Program Execution-----------------------==" << endl;
