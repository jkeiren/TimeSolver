...
extern int yyparse(bool debug, pes& input_pes, int& MAXC,
 ...
 * @return None */
void
yyerror(bool /*debug*/, pes&, int& /*MAXC*/,
        int& /*predicateInd*/, DBM*& /*InitC*/,
        bidirectional_map<std::string, int>* /* declared_atomic*/,
...
int main(int argc, char** argv){
   ...
   * (usually). */
  int parseError = yyparse(opt.debug, input_pes, MAXC,
                           predicateInd, InitC, &input_pes.atomic(),
                           &(input_pes.predicates()), &input_pes.initially(), &(input_pes.equations()));
   ...
   */
  ExprNode* start = lookup_predicate(input_pes.start_predicate().c_str(), &(input_pes.predicates()));
  ...
  opt.aSize = input_pes.atomic().size();
  ...
  SubstList *sublist = new SubstList(opt.aSize, &input_pes.atomic());
  for(int i=0; i < opt.aSize; i++){
    map <int, int>::iterator it = input_pes.initially().find(i);
  ...
  input_pes.atomic().clear();
  input_pes.initially().clear();
