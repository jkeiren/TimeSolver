...
int main(int argc, char** argv){
  ...
  /* print Starting time */ 
  if(opt.debug) {
    std::cerr <<"Program start time: "<< ctime(&rawtime_lp) ;
  }
  ...
  if(parseError) {
    std::cerr << endl << "**Syntax Error: Error Parsing file.**" << endl << endl;
    std::cerr << "==--End of Program Execution-----------------------==" << endl;
    return 1;
  }
  ...
  std::cerr << "Program input file (timed automaton + MES): " << opt.input_filename << endl;
  std::cerr << "max constant in clock constraints: " << MAXC << endl << endl;
  ...
  if(start == nullptr){
    std::cerr << "Start predicate is not properly given." << endl;
    exit(-1);
  }
  ...
  /* print Starting time */ 
  if(opt.debug) {
    std::cerr << "##--Beginning of Proof==-------------------" << endl;
    std::cerr <<"Prover start time: "<< ctime(&rawtime) ;
  }
  clock_t begin = clock();
  bool suc;
  ...
  if (suc) {
    ...
    std::cerr << "Valid proof found. The timed automaton satisfies the MES." << endl << endl;
  }
  else {
    std::cerr << "--==Program Result:  **Invalid**  ==-----------------" << endl;
    std::cerr << "Invalid proof found. The timed automaton does not satisfy the MES." << endl << endl;
  }
  double totalTime = clockToSecs*(end_lp + begin_lp) + clockToSecs*(end - begin);
  
  std::cerr << "--==Program Time:  **" << clockToSecs*(end_lp + begin_lp) + clockToSecs*(end - begin) << " seconds**  ==----------" << endl;
  std::cerr << "Lexer and parser running time: " << clockToSecs*(end_lp - begin_lp) << " seconds" << endl;
  std::cerr << "Prover running time: " << clockToSecs*(end - begin) << " seconds" << endl;
  std::cerr << "Combined running time: " << totalTime  << " seconds" << endl;
  
  std::cerr << "Number of locations: " << p.getNumLocations() << endl;
