 ...
 * @return None */
void yyerror(bool /*debug*/, pes&, int& /*MAXC*/, DBM*& /*InitC*/, char* s) {
...
int main(int argc, char** argv) {
  ...
  cpplog(cpplogging::info) << "max constant in clock constraints: " << MAXC
                           << endl
                           << endl;
  ...
  if (start == nullptr) {
    cpplog(cpplogging::error)
        << "Start predicate is not properly given." << endl;
    exit(-1);
  }
...
// Use the default clockToSecs value as defined above
  ...
  prover p(input_pes, currParityGfp, prevParityGfp, opt.useCaching, opt.nHash,
           MAXC, opt.nbits);
