...
int main(int argc, char** argv){
  ...
  std::string& start_predicate = input_pes.start_predicate();
  bidirectional_map<std::string, int>* declared_atomic = &input_pes.atomic();
  std::map <int, int>* InitSub = &input_pes.initially();
   ...
   * (usually). */
  int parseError = yyparse(opt.debug, &(input_pes.transitions()), input_pes.invariants(), MAXC, start_predicate,
                           predicateInd, InitC, &(input_pes.clocks()), declared_atomic,
                           &(input_pes.predicates()), InitSub, &(input_pes.equations()), &spaceDimension);
   ...
   */
  ExprNode* start = lookup_predicate(start_predicate.c_str(), &(input_pes.predicates()));
  ...
  DBM *dbm = new DBM(spaceDimension, &(input_pes.clocks()));
  ...
  if(opt.debug) {
    ...
    for(vector<Transition *>::iterator it = input_pes.transitions().begin();
  ...
  for(vector<ExprNode *>::iterator it = input_pes.invariants().begin();
      it != input_pes.invariants().end(); it++) {
    ExprNode *ls = (*it);
    delete ls;
  }
  // Clear Vectors to free up dynamic memory
  input_pes.invariants().clear();
  
  // Do Extra Work to delete Dynamically Allocated Elements
  for(map<string, ExprNode *>::iterator it = input_pes.equations().begin();
      it != input_pes.equations().end(); it++) {
    ExprNode *ls = (*it).second;
    delete ls;
  }
  input_pes.equations().clear();

  // Do Extra Work to delete Dynamically Allocated Elements
  // Delete the predicates here since the ExprNode Destructor
  // Does not traverse predicates to make sure of no inconsistencies.
  for(map<string, ExprNode *>::iterator it = input_pes.predicates().begin();
      it != input_pes.predicates().end(); it++) {
    ExprNode *ls = (*it).second;
